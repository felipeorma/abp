...
def limpiar_caracteres(df):
    ...
    for col in df_limpio.select_dtypes(include='object').columns:
        df_limpio[col] = df_limpio[col].apply(lambda x:
            unicodedata.normalize('NFKD', str(x)).encode('ascii', 'ignore').decode('utf-8')
        )
    return df_limpio

# Callbacks para dependencias
def actualizar_resultado_si_gol():
    if st.session_state.get("gol_key") == "Sí":
        st.session_state["resultado_key"] = "Gol"

def configurar_para_penal():
    if st.session_state.get("accion_key") == "Penal":
        st.session_state["zona_saque_key"] = "Penal"
        st.session_state["zona_remate_key"] = "Penal"
        st.session_state["primer_contacto_key"] = "N/A"
        st.session_state["parte_cuerpo_key"] = "N/A"
        st.session_state["segundo_contacto_key"] = ""

def configurar_ejecutor_por_equipo():
    if st.session_state.get("equipo_key") == "Rival":
        st.session_state["ejecutor_key"] = "Rival"
...
def mostrar_formulario(lang: str, jugadores, equipos, zonas):
    ...
    with st.container(border=True):
        ...
        datos["Jornada"] = col1.selectbox(get_text(lang, "round"), jornadas, format_func=lambda x: jornadas_tr[x])
        ...
        datos["Condición"] = col3.selectbox(get_text(lang, "condition"), condiciones, format_func=lambda x: condiciones_tr[x])
    ...
    with st.container(border=True):
        ...
        periodo = col1.selectbox(get_text(lang, "period"), periodos, format_func=lambda x: periodos_tr[x])
    ...
    with st.container(border=True):
        ...
        datos["Acción"] = col1.selectbox(get_text(lang, "action_type"), acciones, format_func=lambda x: acciones_tr[x], key="accion_key", on_change=configurar_para_penal)
        datos["Equipo"] = col2.selectbox(get_text(lang, "executing_team"), ["Cavalry FC", "Rival"], key="equipo_key", on_change=configurar_ejecutor_por_equipo)

        if st.session_state.get("equipo_key") == "Cavalry FC":
            datos["Ejecutor"] = col3.selectbox(get_text(lang, "executor"), jugadores, key="ejecutor_key")
        else:
            datos["Ejecutor"] = "Rival"
            col3.text_input(get_text(lang, "executor"), value="Rival", disabled=True, key="ejecutor_key")
    ...
    with st.container(border=True):
        ...
        if st.session_state.get("accion_key") == "Penal":
            ...
            col1, col2 = st.columns(2)
            datos["Zona Saque"] = col1.selectbox(get_text(lang, "kickoff_zone"), zona_opciones, format_func=lambda x: zonas_tr[x], key="zona_saque_key")
            datos["Zona Remate"] = col2.selectbox(get_text(lang, "shot_zone"), zona_opciones, format_func=lambda x: zonas_tr[x], key="zona_remate_key")

            opciones_contacto = jugadores + [get_text(lang, "opponent")]
            datos["Primer Contacto"] = st.selectbox(get_text(lang, "first_contact"), opciones_contacto, key="primer_contacto_key")

            partes_cuerpo = ["Cabeza", "Otro", "Pie"]
            partes_tr = get_localized_options(lang, partes_cuerpo, "body_part")
            datos["Parte Cuerpo"] = st.selectbox(get_text(lang, "body_part"), partes_cuerpo, format_func=lambda x: partes_tr[x], key="parte_cuerpo_key")
            ...
            segundo_contacto = st.selectbox(get_text(lang, "second_contact"), segundo_opciones, format_func=lambda x: segundo_tr.get(x, x), key="segundo_contacto_key")
    ...
    with st.container(border=True):
        ...
        datos["Gol"] = col1.selectbox(get_text(lang, "is_goal"), gol_opciones, format_func=lambda x: gol_tr[x], key="gol_key", on_change=actualizar_resultado_si_gol)
        ...
        if st.session_state.get("gol_key") == "Sí":
            datos["Resultado"] = "Gol"
            col1.text_input(get_text(lang, "final_result"), value=resultados_tr["Gol"], disabled=True, key="resultado_key")
        else:
            datos["Resultado"] = col1.selectbox(get_text(lang, "final_result"), resultados, format_func=lambda x: resultados_tr[x], key="resultado_key")

        perfiles = ["Hábil", "No hábil"]
        perfiles_tr = get_localized_options(lang, perfiles, "profile")
        datos["Perfil"] = col2.selectbox(get_text(lang, "executor_profile"), perfiles, format_func=lambda x: perfiles_tr[x])

        estrategias = ["Sí", "No"]
        estrategias_tr = get_localized_options(lang, estrategias, "strategy")
        datos["Estrategia"] = col2.selectbox(get_text(lang, "strategy"), estrategias, format_func=lambda x: estrategias_tr[x])

        ejecuciones = ["Centro", "Pase corto", "Disparo directo"]
        ejecuciones_tr = get_localized_options(lang, ejecuciones, "exec_type")
        datos["Tipo Ejecución"] = col2.selectbox(get_text(lang, "execution_type"), ejecuciones, format_func=lambda x: ejecuciones_tr[x])
...
def generar_heatmaps(lang: str, df, zonas):
    try:
        ...
        pitch.draw(ax=ax1)
        pitch.kdeplot(df['x_saque'], df['y_saque'], ax=ax1, cmap='Greens', **heatmap_params)
        ...
        pitch.draw(ax=ax2)
        pitch.kdeplot(df['x_remate'], df['y_remate'], ax=ax2, cmap='Reds', **heatmap_params)
        ax2.set_title(get_text(lang, "shot_zones"), fontsize=16, pad=20, fontweight='bold')
        st.pyplot(fig2)

        # Exportar CSV limpio
        df_limpio = limpiar_caracteres(df)
        csv = df_limpio.to_csv(index=False, encoding='utf-8-sig').encode('utf-8-sig')
