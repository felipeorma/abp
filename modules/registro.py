...
def registro_page(lang: str):
    # Inicializar registro en session_state si no existe
...
def cargar_datos():
    jugadores = sorted([
        ...
        "Myer Bevan", "Opponent"
    ], key=lambda x: x.split()[-1]) + ["Marco Carducci", "Opponent"]
...
def mostrar_formulario(lang: str, jugadores, equipos, zonas):
    ...
    with st.container(border=True):
        ...
        if datos["Acción"] == "Penal":
            ...
            datos["x_saque"], datos["y_saque"] = zonas["Penal"]
            datos["x_remate"], datos["y_remate"] = zonas["Penal"]
            ...
            # Añadir coordenadas
            datos["x_saque"], datos["y_saque"] = zonas[datos["Zona Saque"]]
            datos["x_remate"], datos["y_remate"] = zonas[datos["Zona Remate"]]
