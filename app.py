...
def analytics_page():
    ...
    def load_github_data(url):
        try:
            ...
            # Preprocesamiento adaptado a tus datos reales
    ...
    if not df.empty:
        ...
        # Filtro temporal (solo si existe la columna Fecha)
        if 'Fecha' in df.columns:
        ...
        # Filtros adaptados a tus columnas reales
        available_filters = {
            'Rival': df['Rival'].unique(),
            'Ejecutor': df['Ejecutor'].unique(),
            'Acción': df['Acción'].unique()
        }
        ...
        kpis = {
            ...
            "Estrategias Usadas": f"{filtered_df['Estrategia'].eq('Sí').sum()}",
            "Goles Totales": filtered_df['Gol'].eq('Sí').sum()
        }
        ...
        # ========== ANÁLISIS DE OPONENTES CORREGIDO ==========
        st.header("🆚 Análisis de Oponentes")
        
        try:
            opponent_analysis = filtered_df.groupby('Rival').agg(
                Acciones_Contra=('Acción', 'count'),
                Goles_Recibidos=('Gol', lambda x: sum(x == 'Sí')),  # PARÉNTESIS CORREGIDO
                Zona_Peligrosa=('Zona Remate', lambda x: x.mode()[0] if not x.empty else 'N/A')
            ).reset_index()
            ...
            fig = px.treemap(
                ...
                title='Acciones Defensivas por Oponente'
            )
            st.plotly_chart(fig, use_container_width=True)
            
        except Exception as e:
            st.error(f"Error en análisis de oponentes: {str(e)}")
